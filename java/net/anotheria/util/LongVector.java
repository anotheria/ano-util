...
/**
 * The <code>LongVector</code> class implements a growable array of
...
public
class LongVector implements Cloneable, java.io.Serializable {
	/**
	 ...
	 * @since      JDK1.0
	 */
	public final synchronized long removeElementAt(int index) {
	if (index >= elementCount) {
		throw new ArrayIndexOutOfBoundsException(index + " >= " +
							 elementCount);
	}
...
final
class LongVectorEnumerator implements java.util.Enumeration {
	LongVector vector;
	int count;

	LongVectorEnumerator(LongVector v) {
	vector = v;
	count = 0;
	}
	...
	synchronized (vector) {
		...
		return new Long(vector.elementData[count++]);
		}
	}
...
/* ------------------------------------------------------------------------- *
  ...
  *** empty log message ***
  ...
  Revision 1.5  2001/01/15 15:15:21  weg
  no message

  Revision 1.2  2000/12/13 18:13:37  hwa
  Added header and footer for CVS
